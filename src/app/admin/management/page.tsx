...
function AdminManagementContent() {
    ...
    const [csvTargetType, setCsvTargetType] = useState<'company' | 'job' | null>(null);
    ...
    const handleCsvUpload = async (event: React.ChangeEvent<HTMLInputElement>, type: 'company' | 'job') => {
        ...
        reader.onload = (e) => {
            ...
            const targetFields = type === 'company' ? COMPANY_FIELDS : JOB_FIELDS;

            headers.forEach(header => {
                // Try to find a matching key or label
                const match = Object.entries(targetFields).find(([key, label]) =>
                    header === key || header === label || label.includes(header)
                );
                if (match) {
    ...
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <div className="min-h-screen bg-slate-50 pb-20">
                ...
                {/* Confirm Modify at start of AdminManagementContent return */}
